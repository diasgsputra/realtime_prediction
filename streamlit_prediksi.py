...
genre = st.radio(
     "ID Alat :",
     ('C001 - Wonosari', 'C002 - Bantul', 'C003'))

if genre == 'C001 - Wonosari':
    ...
    while True :
        ...
        mycursor = mydb.cursor()
        mycursor.execute("SELECT t5,t4,t3,t2,t1,h,t,ch FROM realtime_detik_2 ORDER BY id DESC LIMIT 1")
        realtime = mycursor.fetchone()
        ...
        ##################################### PREDIKSI MENIT #####################################
        mycursor = mydb.cursor()
        mycursor.execute("SELECT t5,t4,t3,t2,t1,h,t,ch FROM realtime_menit_2 ORDER BY id DESC LIMIT 1")
        mrealtime = mycursor.fetchone()
        #print("mrealtime: ",mrealtime)
        # mrealtime = konek.ambil_menit()
        mrealtime = np.array(mrealtime)
        ...
        ##################################### PREDIKSI JAM #####################################
        mycursor = mydb.cursor()
        mycursor.execute("SELECT t5,t4,t3,t2,t1,h,t,ch FROM realtime_jam_2 ORDER BY id DESC LIMIT 1")
        jrealtime = mycursor.fetchone()
        #print("mrealtime: ",mrealtime)
        # mrealtime = konek.ambil_menit()
        jrealtime = np.array(jrealtime)
